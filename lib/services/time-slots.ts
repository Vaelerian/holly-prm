...
export async function listTimeSlotsForRange(
  ...
  const concreteSlots = await prisma.timeSlot.findMany({
    ...
    include: {
      tasks: { select: { id: true, title: true, effortSize: true, scheduleState: true, projectId: true, roleId: true } },
    },
  })
  ...
  const resolved: ResolvedTimeSlot[] = concreteSlots.map(s => ({
    ...
    assignedTasks: (s.tasks ?? []).map((t: { id: string; title: string; effortSize: string; scheduleState: string; projectId: string | null; roleId: string }) => ({
      id: t.id,
      title: t.title,
